---
title: "Convolutional architecture on image dataset"
...
```
The KMNIST dataset consists of grayscale images of 28x28 pixels. Instead of digits as in the classic MNIST dataset,
KMNIST consists of Japanese Hiragana characters. It has 70000 samples. Here, we will demonstrate how to apply 
LBBNNs to a convolutional architecture. 


## Install torchvision if needed and transform data to ensure it works on all platforms
...
## Define dataloaders via the torchvision dataset
...
Here we have to manually define the layers, as we will use both convolutional and feed-forward layers.
...
These functions work the same as with feed-forward architectures. It is recommended to train on
GPU if one wants to obtain excellent performance on this dataset, as it will require a few hundred epochs
of training, which will be very slow on CPU. 
```{r}
train_lbbnn(epochs = 2, LBBNN = model_kmnist, lr = 0.01, train_dl = train_loader_kmnist,
            device = device)
